...
//add for himax
int himax_flash_write_burst(uint8_t * reg_byte, uint8_t * write_data)
{
	int ret =0;
    uint8_t data_byte[8];
    int i = 0;
	...
	ret = himax_bus_write(0x00, 8, data_byte);
    if (ret < 0) {
...
int mpTestFunc(struct chip_data_hx83112b *chip_info, uint8_t checktype, uint32_t datalen)
{
    ...
    RAW = (uint32_t*)kmalloc(sizeof(uint32_t)*len,GFP_KERNEL);
	if(RAW == NULL){
		TPD_INFO("kamlloc fialed,Raw is null\n");
		return RESULT_ERR;
	}
#ifdef RAWDATA_NOISE
    uint32_t RAW_Rawdata[len];
#endif
...
void himax_ts_diag_func(struct chip_data_hx83112b *chip_info, int32_t *mutual_data)
{
    ...
    int dsram_type = 0;
    //char temp_buf[20];
    //char write_buf[6000 * 3];

    //memset(write_buf, '\0', sizeof(write_buf));
    total_size = chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM * 2;
    if(info_data ==NULL)
		return;
    info_data = kmalloc(total_size*sizeof(char),GFP_KERNEL);
...
static int hx83112b_enable_charge_mode(struct chip_data_hx83112b *chip_info, bool enable)
{
    ...
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
    TPD_INFO("%s, TP charge mode enable = %d\n", __func__, enable);
    ...
    if(enable)
    {
       ...
       tmp_data[1] = 0xA5;
       tmp_data[0] = 0x5A;
       ret = himax_flash_write_burst(tmp_addr, tmp_data);
    }
    else
    {
       tmp_addr[3] = 0x10;
