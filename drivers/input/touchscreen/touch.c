...
int tp_util_get_vendor(struct hw_resource *hw_res, struct panel_info *panel_data)
{
	...
	switch(get_project()) {
		...
		case 19700:
			prj_id = 19751;
			pr_err("use TP fw directory 19751\n");
			break;
		case 20620:
		case 20621:
		case 20622:
		case 20690:
			prj_id = 20621;
			pr_err("use TP fw directory 26021\n");
			break;
		default:
		pr_err("can not find TP fw directory\n");	
		break;
	}
	...
	if (tp_used_index == himax_83112a) {
		...
		if(prj_id == 19751)
			...
			memcpy(panel_data->manufacture_info.version, "NT_JDIL", 7);
