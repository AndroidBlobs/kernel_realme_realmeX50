...
static void ufshcd_auto_hibern8_update(struct ufs_hba *hba, u32 ahit)
{
	...
	if (!pm_runtime_suspended(hba->dev)) {
		...
		down_write(&hba->lock);
#ifdef VENDOR_EDIT
		trace_ufs_scsi_block_rq("ufshcd_auto_hibern8_update ufshcd_scsi_block_requests\n");
#endif
		...
		up_write(&hba->lock);
#ifdef VENDOR_EDIT
		trace_ufs_scsi_block_rq("ufshcd_auto_hibern8_update ufshcd_scsi_unblock_requests\n");
#endif
