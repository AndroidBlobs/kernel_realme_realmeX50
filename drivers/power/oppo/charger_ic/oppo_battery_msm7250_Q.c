...
/* tongfeng.Huang@BSP.CHG.Basic, 2018/12/07,  Add for suspending*/
	...
	if (suspend)
		vote(chg->icl_irq_disable_votable, USB_SUSPEND_VOTER,
				true, 0);
#ifdef VENDOR_EDIT
/* OuYangBaiLi@BSP.CHG.Basic,charging_bsp 2019/11/20,Add for shortc charging_bsp */
	if(usb_status)
		suspend = true;
#endif /* VENDOR_EDIT */
...
static int oppo_usbtemp_monitor_main(void *data)
{
	...
	int print_temp = 0,print_big = 0;
	#ifndef CONFIG_HIGH_TEMP_VERSION/* OuYangBaiLi@BSP.CHG.Basic,charging_bsp 2019/11/07,Add for charging_bsp */
	...
	while (!kthread_should_stop()) {
		...
		if(chip->usb_temp_l > chip->usb_temp_r){
			print_big = chip->usb_temp_l;
		}else {

			print_big = chip->usb_temp_r;
		}
		if(print_big != print_temp)
			chg_err("usb_tem_change usb_temp_r:%d,usb_temp_l:%d\n",chip->usb_temp_r, chip->usb_temp_l);
		print_temp = print_big;
		#endif /* VENDOR_EDIT */
				...
				if (dischg_flag == false) {
					...
					if (count_r >= retry_cnt || count_l >= retry_cnt) {
						if (!IS_ERR_OR_NULL(chip->normalchg_gpio.dischg_enable) && !(get_project() == 19101 && strcmp("T0",get_PCB_Version()) == 0)) {
							...
							vote(chip->pmic_spmi.smb5_chip->chg.usb_icl_votable, USB_PSY_VOTER, true, 0);
...
#else
							chg_err(" set dischg_enable pull down vbus\n");
